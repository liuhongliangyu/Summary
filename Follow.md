...
将摄像机作为要跟随的移动对象的子集，摄像机就会跟随游戏对象移动，但是如果游戏对象是滚动向前的，那么摄像机也会滚动向前，视野混乱。不建议采取这种方法。
...
```C#
public GameObject player;  //持有跟随目标的游戏对象

Vector3 offset;  //启动时，目标对象的位置与摄像机的位置的差值
void Start()
{
  player = GameObject.Find("跟随目标对象");
  offset = this.transform.position - player.transform.position;   //摄像机与跟随目标的位置差值
}

void Update()
{
  this.transform.position = player.transform.position + offset;   //每一帧都改变摄像机的位置，使摄像机的位置为跟随目标的位置与offset的和
}
```

优点：代码简单，但是不能跟随人物旋转，运动。

将脚本挂载到摄像机身上，当然也可以挂载到跟随目标上，那么这里的变量就要变化一下了，根据具体情况在更改。
